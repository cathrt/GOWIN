/*
角速度生成模块
采用T法测速，公式：角速度 = K / m2，
其中m2为脉冲间隔计数值，K_MOTOR为常数，f_c为采样频率(时钟频率), 单位为rad/s
*/
module decode_speed (
        input  wire clk,
        input  wire rst_n,
        //输入
        input  wire encode_pluse,               //边沿脉冲
        input  wire motor_dir,                  //电机转向,[1]为正转，[0]为反转
        //输出
        output reg  signed [15:0] cur_speed     //当前角速度
    );

    // K = 2 * Π * f_c / 1040 ≈ 302076
    localparam integer K = 19'd302076;
    //两个脉冲之间间隔时间比20ms长，则认为电机不转，50MHz*20ms = 1_000_000 
    localparam integer CNT_MAX = 20'd1_000_000;
    //自动计算位宽
    localparam WIDTH = $clog2(CNT_MAX);

    //计数器，用于计算脉冲间隔计数值
    reg [WIDTH-1:0] cnt; 
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            cnt <= CNT_MAX;   //上电时不让电机转动
        end else if (encode_pluse) begin
            cnt <= 'd1; //脉冲到来，计数器清零,但清零的这个周期应该是新周期的第一个系统周期
        end else if (cnt < CNT_MAX) begin
            cnt <= cnt + 1'b1; //脉冲间隔计数值小于最大值，计数器加1
        end else begin
            cnt <= CNT_MAX; //脉冲间隔计数值大于最大值，计数器保持最大值
        end
    end

    //超过脉冲间隔计数值最大值，使能电机停止转动
    reg motor_stop;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            motor_stop <= 1'b1; //上电关闭电机
        end else if (cnt >= CNT_MAX) begin
            motor_stop <= 1'b1; //超限关闭电机
        end else if(encode_pluse) begin
            // 脉冲到来：若之前未超时(cnt < CNT_MAX)，说明是连续脉冲，速度生效；
            // 若之前已超时(cnt >= CNT_MAX)，说明是起步首脉冲，速度暂不可信
            motor_stop <= (cnt < CNT_MAX);
        end
    end

    //锁存脉冲间隔计数值
    //不锁存的话，cnt会直接清零重新计数
    reg [WIDTH-1:0] m2_r;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            m2_r <= 'd1; //除数不能为0，初始化为1
        end else if (encode_pluse && cnt < CNT_MAX) begin
            m2_r <= cnt; //脉冲到来，锁存脉冲间隔计数值
        end 
    end

    // Gowin Divider IP 除法器 例化
    wire [18:0]speed;
    gowin_divider u_gowin_divider (
        .clk        (clk),          // 50MHz 系统时钟
        .restn      (rst_n),        // 高云 IP 复位低电平有效
        .dividend   (K),            // 被除数输入(常数)
        .divisor    (m2_r),         // 除数输入（测得的脉冲间隔计数值）
        .quotient   (speed)         // 输出的商（角速度）
    );

    // 角速度符号处理与饱和截断，将19位speed转换为16位，最大16位：32767
    wire [15:0] speed_clamped = (speed > 18'd32767) ? 16'd32767 : speed[15:0];

    //电机正反转
    //除法器不允许除数为0，而当电机停止转动时，m2_r被设置为1，除法器输出的角速度为K_MOTOR，但实际上此时角速度应为0，因此需要在此处进行处理
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            cur_speed <= 16'sd0;
        end else if (motor_stop) begin
            cur_speed <= 16'sd0; //强制电机停止转动，角速度为0
        end else if (motor_dir) begin
            cur_speed <= speed_clamped; //正转，角速度为正
        end else begin
            cur_speed <= -speed_clamped; //反转，角速度为负
        end
        
    end
endmodule